...
\centering
        ...
        \vspace{1em}
        \caption[Architecture of the used UNet model]{\textbf{The architecture of the model}, which is strongly inspired by the 3D-UNet models proposed in \cite{lee2017superhuman,funke2018large}. 
...
LFC\cite{parag2017anisotropic} & \CrossedBox & \HollowBox & \HollowBox & Z-Watershed+Agglo & \HollowBox & 0.616  \\
        \end{tabular}
        \vspace*{0.99em}
    \caption[Achieved scores on the CREMI neuron segmentation challenge]{Representative excerpt of the published methods currently part of the CREMI leaderboard \cite{cremi}. The best method proposed here achieves competitive scores and is based on an efficient parameter-free algorithm that does not rely on superpixels. For more details about the partitioning algorithms used by related work, see references in the first column.}
    \label{tab:test_results}
\end{minipage}
\end{table}
...
-               & (0, -12, 0)   & - & - \\
        ...
        % }
        \vspace{3em}
        \caption[Sparse neighborhood structures]{\textbf{Sparse neighborhood structures} --  
